...
function getOrderedFeaturedProjects(projects: readonly Project[]) {
  return [...projects].sort((firstProject, secondProject) => {
    const firstIndex = preferredFeaturedProjectOrder.indexOf(
      firstProject.id as (typeof preferredFeaturedProjectOrder)[number]
    );
    const secondIndex = preferredFeaturedProjectOrder.indexOf(
      secondProject.id as (typeof preferredFeaturedProjectOrder)[number]
    );
...
function repositoryMatchesFilter(
  ...
  const filterKeywords: Record<Exclude<ProjectCategory, "all">, string[]> = {
    ...
    desktop: ["desktop", "avalonia", "mvvm", "odbc", "pharmacy"],
    algorithms: ["algorithm", "algorithms", "benchmark", "tsp", "atsp", "cpp", "c++"]
  };
...
function formatRepositoryDate(updatedAt: string | null, locale: Locale) {
  ...
  const month = new Intl.DateTimeFormat(locale === "pl" ? "pl-PL" : "en-US", {
    month: "short"
  })
    .format(date)
    .replace(".", "");
...
export function PortfolioExperience({
  ...
  const featuredProjects = useMemo(
    () => getOrderedFeaturedProjects(content.projects),
    [content.projects]
  );
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col">
          ...
          >
            ...
            <p
              className={cn(
                "project-card-teaser mt-3 text-sm leading-6 md:text-[0.95rem]",
    ...
    <motion.article className="featured-project-detail card h-full p-4 md:p-6">
      ...
      >
        <div>
          ...
          <section className="project-detail-panel project-detail-technical mt-4 rounded-lg border border-soft bg-white/65 p-4">
          ...
          <section className="project-gallery project-detail-gallery mt-4 rounded-lg border border-soft bg-white/55 p-3">
